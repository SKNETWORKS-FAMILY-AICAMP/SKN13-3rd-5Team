...
### 9️⃣ 성능 개선 노력
- **메모리 캐시 활용**: 사용자 session_id별로 객체를 한 번 생성한 뒤,이를 메모리에 캐시하여 이후부터는 즉시 재사용하도록 구조를 변경하여 응답속도를 향상시키고
불필요한 반복 처리를 줄여 전체 처리효율을 높임
- **Chroma 벡터 데이터베이스 도입**: 대규모 문서 검색 효율을 개선하고 의미 기반 정보 검색을 가능하게 하기 위해, 벡터 임베딩 기반의 Chroma DB를 구성하여 응답 정확도와 속도 모두를 향상시킴
- **LangGraph 기반 구조 전환**: 기존의 체인(Chain) 기반 직렬 처리 흐름에서 LangGraph 구조로 전환하여, Tool 병렬 실행과 유연한 제어 흐름을 통해 전체 처리 속도 및 응답 효율을 높임
